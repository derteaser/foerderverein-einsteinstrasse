...
- `site/templates/*.blade.php` — top-level Blade templates (`home`, `blog`, `article`, `contact`, `default`, `person`). Each wraps content in `<x-layout>`. `persons.php` stays as raw PHP — it's just a `go('/', 301)` redirect.
- `site/templates/components/layout.blade.php` — global HTML shell rendered as `<x-layout>`. Emits `@snippet('seo/head')` (from `tobimori/kirby-seo`) in `<head>`, owns charset/viewport/theme-color/icons/vite tags/RSS link. Accepts a `:show-header-navigation="false"` prop (used by the home template, which has its own hero).
- `site/snippets/*.blade.php` — reusable Blade partials. The home page is composed from child pages whose `intendedTemplate()` matches a snippet name (`pager-hero`, `pager-steps`, `pager-cta`, `pager-blog`, `pager-team`, `pager-features`). Adding a new home section = new blueprint + matching snippet.
- `site/snippets/icons/*.php` and `site/snippets/feed/rss.php` stay as raw PHP — the former are auto-generated by `copy-icons.cjs`, the latter is the `bnomei/kirby3-feed` integration. `@snippet('icons/foo', [...])` works from Blade against `.php` partials thanks to `leitsch/kirby-blade`'s fallback resolver.
