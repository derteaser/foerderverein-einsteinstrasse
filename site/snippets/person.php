...
?>
<div class="w-full lg:w-1/2 px-8 mb-8 lg:mb-20">
  <div class="relative p-6 lg:p-12 bg-purple-900">
    <div class="flex flex-wrap items-center -mx-4">
      ...
      <div class="w-full md:w-1/3 px-4">
        <div class="lg:absolute top-0 right-0 -mt-4 -mr-4">
          <?php if ($image = $person->main_image()->toFile()): ?>
            <?php $resizedImage = $image->thumb('person') ?>
            <img class="w-64 h-64 rounded-full object-cover mx-auto" src="<?= $resizedImage->url() ?>" width="<?= $resizedImage->width() ?>" height="<?= $resizedImage->height() ?>" alt="Foto von <?= $person->title() ?>">
          <?php endif ?>
        </div>
      </div>
    </div>
